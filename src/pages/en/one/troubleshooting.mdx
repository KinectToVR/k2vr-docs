...
### Boot looping
There are two primary causes of boot looping for an Xbox One Kinect: either Windows Audio Enhancements or insufficient USB bandwidth.

For the first one, go into Windows Settings, then System > Sound > More Sound Settings > Recording (top bar) > Microphone Array "Xbox NUI Sensor" > Properties > Advanced (top bar) > **uncheck "Enable audio enhancements**.

*The Kinect uses multiple microphones to have a rough idea of where you are. This option can screw up the Kinect drivers.*

For the second one, **changing USB port where the Kinect is plugged in and unplugging everything else** may solve the issue. Make sure you've plugged the Kinect adapter in a USB 3.0 port directly in your computer and NOT into an external USB hub.

*The Kinect needs almost full USB 3.0 bandwidth capacity provided by a single USB controller. Your PC may or may not have multiple capable USB controllers that control different USB ports.*

If you need further help with this, you can [join our Discord](https://discord.gg/CzVCfv5s6Z).
...
The video is now private, but there are plenty of teardowns and possible alternative solutions on YouTube. You can check those!
